...
export class AgentViewProvider implements vscode.WebviewViewProvider {
    ...
    private getHtmlContent(webview: vscode.Webview): string {
        ...
        return [
            ...
            '                if (currentAgentBubble && currentAgentBubble.parentNode === currentTurnWrapper) {',
            '                    currentTurnWrapper.insertBefore(currentActivityAccordion, currentAgentBubble);',
            '                } else {',
            '                    currentTurnWrapper.appendChild(currentActivityAccordion);',
            '                }',
            '            }',
            '        }',
            ...
            '                }',
            '                if (currentAgentBubble && (!currentAgentText || currentAgentText.trim().length === 0)) {',
            '                    if (currentToolRows.size > 0 || (currentTurnWrapper && currentTurnWrapper.querySelector(".diff-card, .terminal-card"))) {',
            ...
            '                case "turn_complete": {',
            '                    setRunningState(false);',
            '                    if (msg.response && typeof msg.response === "string" && msg.response.trim().length > 0) {',
            '                        if (!currentAgentText || currentAgentText.trim().length === 0 || currentAgentText !== msg.response) {',
            '                            currentAgentText = msg.response;',
            '                            if (!currentAgentBubble) {',
            '                                if (currentTurnWrapper) {',
            '                                    currentAgentBubble = document.createElement("div");',
            '                                    currentAgentBubble.className = "agent-bubble";',
            ...
            '                            currentAgentBubble.innerHTML = renderMarkdown(currentAgentText);',
            '                        }',
            '                    } else if (!currentAgentText || currentAgentText.trim().length === 0) {',
            '                        if (currentToolRows.size > 0 || (currentTurnWrapper && currentTurnWrapper.querySelector(".diff-card, .terminal-card"))) {',
            '                            if (currentAgentBubble) {',
            '                                currentAgentBubble.remove();',
            '                                currentAgentBubble = null;',
            ...
            '                            if (msg.status === "ERROR") {',
            '                                currentAgentBubble.innerHTML = \'<span style="color: var(--vscode-errorForeground);">⚠️ Request ended with an error.</span>\';',
            '                            } else {',
            '                                currentAgentBubble.innerHTML = \'<span style="color: var(--vscode-descriptionForeground);">(Completed with no output)</span>\';',
            '                            }',
            '                        }',
            '                    }',
            '                    scheduleMermaidRender(0);',
